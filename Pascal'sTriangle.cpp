...
{
	if (row < 0 || pos < 0 || pos > row) throw 1; //check input for illegal coord
	if (pos == 0 || pos == row) return 1; //check if edge of row
	//recursive call
	...
	{
    unsigned __int64 answer = pascal_val(row - 1, pos - 1, triangle) + 
        pascal_val(row - 1, pos, triangle);
